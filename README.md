...
1. **Подключение БД (MySQL) и использование Hibernate/Spring Data JPA**  
   ...
   - Связи:  
     - `Order` ↔ `Smartphone` (один ко многим / многие к одному)  
     - `User` ↔ `Smartphone` (многие ко многим)  
   - Настроены каскадные операции и ленивые/жадные загрузки.  

2. **Добавление in-memory кеша**  
   - Для кастомного GET-запроса с параметрами (по `customerName` заказа) результаты запросов к БД кешируются в `ConcurrentHashMap`.  
   - При повторном обращении с тем же ключом (`customerName`) данные возвращаются из кеша.  
...
- **In-memory кеш** для ранее запрошенных данных.
